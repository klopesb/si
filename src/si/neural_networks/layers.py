...
class Dropout(Layer):
    ...
    Dropout is a regularization technique where a random set of neurons
    is temporarily ignored (dropped out) during training, helping prevent
    overfitting by promoting robustness and generalization in the model.
    
    Parameters
    ----------
    probability : float
        The dropout rate, between 0 and 1. This is the probability that
        a neuron will be dropped (set to zero) during training.
    ...
    def forward_propagation(self, input: np.ndarray, training: bool) -> np.ndarray:
        ...
        Applies the mask to the input and scales it when in training mode.
        Does nothing in inference mode (returns the received input).
    ...
    def backward_propagation(self, output_error: np.ndarray) -> np.ndarray:
        ...
        -------
        np.ndarray
            The input error (output_error * mask)
        """
